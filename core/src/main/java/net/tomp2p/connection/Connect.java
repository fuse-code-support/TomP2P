...
public class Connect {
	...
	@Getter
	private final CountConnectionOutboundHandler counterUDP = new CountConnectionOutboundHandler();
	@Getter
	private final CountConnectionOutboundHandler counterTCP = new CountConnectionOutboundHandler();

	// this map caches all messages which are meant to be sent by a reverse
	// connection setup
	private final ConcurrentCacheMap<Integer, Message> cachedRequests = new ConcurrentCacheMap<Integer, Message>(30,
			1024);
	...
	// final FutureDone<Pair<PeerConnection,Message>> future = new
	// FutureDone<Pair<PeerConnection,Message>>();
	public SendBehavior.SendMethod connectTCP(final SimpleChannelInboundHandler<Message> replHandler,
			final int connectTimeoutMillis, final PeerAddress sender, final PeerConnection peerConnection,
			final boolean isReflected) {
		...
		if (peerConnection.isExisting() || peerConnection.isOpen()) {
			...
			case DIRECT:
				Pair<ChannelCreator.ChannelCloseListener, ChannelFuture> pair1 = createChannelTCP(
						peerConnection.remotePeer().createTCPSocket(sender), peerConnection.channelCreator(),
						replHandler, timeoutHandler, connectTimeoutMillis, peerConnection.isKeepAlive());
				if (pair1 == null) {
			...
			case RCON:
				Pair<ChannelCreator.ChannelCloseListener, ChannelFuture> pair2 = createChannelUDP(
						peerConnection.remotePeer().createUDPSocket(sender), peerConnection.channelCreator(), null,
						timeoutHandler, true);
			 ...
			 * if(peerSocketAddress == null) { throw new
			 * IllegalArgumentException("Illegal sending behavior: no relay provided, but relay indicated RCON"
			 * ); } Message rconMessage = createRconMessage(peerSocketAddress,
			 * message);
			 * if(peerSocketAddress.equals(peerBean.serverPeerAddress().
			 * ipv4Socket())) { LOG.debug("Send to self-relay RCON"); return new
			 * Pair<PeerConnection,Message>(SELF_PEERCONNECTION_MARKER,
			 * rconMessage); } return handleRcon(peerSocketAddress, message,
			 * rconMessage, channelCreator, connectTimeoutMillis,
			 * peerConnection);
			 */
			/*
			 * case HOLEP_RELAY: //handleHolePunch(futureResponse, message,
			 * channelCreator, idleTCPMillis, handler, true, handler,
			 * channelFuture); return doRelayFallbackTCP(replHandler,
			 * futureResponse, message, channelCreator, connectTimeoutMillis,
	 ...
	 */
	/*
	 * private Pair<PeerConnection,Message> handleRcon(PeerSocketAddress ps,
	 * final Message message, final Message rconMessage, final ChannelCreator
	 * channelCreator, final int connectTimeoutMillis, final PeerConnection
	 * peerConnection) {
	 * LOG.debug("initiate reverse connection setup to peer with peerAddress {}"
	 * , message.recipient()); message.keepAlive(true);
	 * 
	 * 
	 * // cache the original message until the connection is established
	 * cachedRequests.put(message.messageId(), rconMessage);
	 * 
	 * SimpleChannelInboundHandler<Message> rconInboundHandler = new
	 * SimpleChannelInboundHandler<Message>() {
	 * 
	 * @Override protected void channelRead0(ChannelHandlerContext ctx, Message
	 * msg) throws Exception { if (msg.command() == Commands.RCON.getNr() &&
	 * msg.type() == Type.OK) {
	 * LOG.debug("Successfully set up the reverse connection to peer {}",
	 * message.recipient().peerId()); } else {
	 * LOG.debug("Could not acquire a reverse connection, msg: {}", message);
	 * cachedRequests.remove(message.messageId()); } } };
	 * 
	 * // send reverse connection request instead of normal message return
	 * connectTCP(rconInboundHandler, rconMessage, channelCreator,
	 * connectTimeoutMillis, connectTimeoutMillis, peerConnection);
	 * 
	 * }
	 */
	 ...
	 * @param messageType
	 */
	private static void readyToSend(final Message originalMessage, PeerSocketAddress socketAddress, Message newMessage,
			byte RPCCommand, Type messageType) {
		...
		if (socketAddress instanceof PeerSocket4Address) {
			...
			psa = psa.withIpv4(socketAddress4.ipv4()).withTcpPort(socketAddress4.tcpPort())
					.withUdpPort(socketAddress4.udpPort());

		}

		PeerAddress recipient = originalMessage.recipient().withIpv4Socket(psa).withRelaySize(0);
	...
	private Pair<ChannelCreator.ChannelCloseListener, ChannelFuture> createChannelTCP(InetSocketAddress recipient,
			ChannelCreator channelCreator, ChannelHandler handler, IdleStateHandler timeoutHandler,
			int connectTimeoutMillis, boolean isKeepAlive) {
	...
	public SendBehavior.SendMethod connectUDP(final SimpleChannelInboundHandler<Message> handler,
			final ChannelCreator channelCreator, final PeerAddress sender, final PeerConnection peerConnection,
			final boolean isReflected, final boolean broadcast) {
		...
		case DIRECT:
			...
			Pair<ChannelCreator.ChannelCloseListener, ChannelFuture> pair = createChannelUDP(
					peerConnection.remotePeer().createUDPSocket(sender), channelCreator, handler, timeoutHandler,
					isFireAndForget);
			peerConnection.channelFuture(pair.element1(), pair.element0());
			return SendBehavior.SendMethod.DIRECT;

		/*
		 * case HOLEP_RELAY: if (peerBean.holePunchInitiator() != null) {
		 * handleHolePunch(futureResponse, message, channelCreator,
		 * idleUDPMillis, handler, broadcast, handlers); // all the send
		 * mechanics are done in a // AbstractHolePuncherStrategy class. //
		 * Therefore we must execute this return statement. return; } LOG.
		 * debug("No hole punching possible, because There is no PeerNAT. New Attempt with Relaying"
		 * ); doRelayFallbackUDP(futureResponse, message, broadcast, handlers,
		 * channelCreator, handler); return;
		 */
		case SELF:
	...
	private void handleReflection(final Message message) {
		PeerSocket4Address reflectedRecipient = Utils.natReflection(message.recipient(),
				dispatcher.peerBean().serverPeerAddress());
	...
	/*
	 * private FutureDone<Message> handleHolePunch(final FutureResponse
	 * futureResponse, final Message message, final ChannelCreator
	 * channelCreator, final int idleUDPMillis, final
	 * SimpleChannelInboundHandler<Message> handler, final boolean broadcast,
	 * final Map<String, Pair<EventExecutorGroup, ChannelHandler>> handlers) {
	 * // start hole punching FutureDone<Message> fDone =
	 * peerBean.holePunchInitiator().handleHolePunch(idleUDPMillis,
	 * futureResponse, message); fDone.addListener(new
	 * BaseFutureAdapter<FutureDone<Message>>() {
	 * 
	 * @Override public void operationComplete(FutureDone<Message> future)
	 * throws Exception { if (future.isSuccess()) {
	 * futureResponse.response(future.object()); } else {
	 * LOG.error(future.failedReason()); LOG.
	 * error("Message could not be sent with hole punching! New send attempt with relaying."
	 * ); // futureResponse.failed(future.failedReason()); // throw new
	 * Exception(future.failedReason()); doRelayFallbackUDP(futureResponse,
	 * message, broadcast, handlers, channelCreator, handler); } }
	 * 
	 * @Override public void exceptionCaught(Throwable t) throws Exception { //
	 * futureResponse.failed(t); // throw new Exception(t); LOG.
	 * error("The setup of a connection via has been canceled, because an error was thrown"
	 * ); t.printStackTrace(); doRelayFallbackUDP(futureResponse, message,
	 * broadcast, handlers, channelCreator, handler); }
	 * 
	 * 
	 * }); return fDone; }
	 */
	...
	/*
	 * private void doRelayFallbackUDP(final FutureResponse futureResponse,
	 * final Message message, final boolean broadcast, final Map<String,
	 * Pair<EventExecutorGroup, ChannelHandler>> handlers, final ChannelCreator
	 * channelCreator, final SimpleChannelInboundHandler<Message> handler) {
	 * 
	 * PeerSocketAddress ps = prepareRelaySend(message,
	 * peerBean.serverPeerAddress().ipv4Socket()); if(ps == null) {
	 * futureResponse.failed("no relay provided, but relay indicated HP");
	 * return; } if(ps.equals(peerBean.serverPeerAddress().ipv4Socket())) {
	 * LOG.debug("Send to self-relay HP"); sendSelf(futureResponse, message);
	 * return; }
	 * 
	 * ChannelFuture channelFuture = channelCreator.createUDP(broadcast,
	 * handlers, futureResponse, false); sendMessage(futureResponse, message,
	 * channelFuture, handler == null); }
	 * 
	 * private void doRelayFallbackTCP(final
	 * SimpleChannelInboundHandler<Message> handler, final FutureResponse
	 * futureResponse, final Message message, final ChannelCreator
	 * channelCreator, final int connectTimeoutMillis, final PeerConnection
	 * peerConnection, final TimeoutFactory timeoutHandler) {
	 * 
	 * PeerSocketAddress ps = prepareRelaySend(message,
	 * peerBean.serverPeerAddress().ipv4Socket()); if(ps == null) {
	 * futureResponse.failed("no relay provided, but relay indicated TCP");
	 * return; } if(ps.equals(peerBean.serverPeerAddress().ipv4Socket())) {
	 * LOG.debug("Send to self-relay TCP"); sendSelf(futureResponse, message);
	 * return; } InetSocketAddress recipient = ps.createTCPSocket();
	 * ChannelFuture channelFuture = sendTCPCreateChannel(recipient,
	 * channelCreator, peerConnection, handler, timeoutHandler,
	 * connectTimeoutMillis, futureResponse); sendMessage(futureResponse,
	 * message, channelFuture, handler == null); }
	 */
	...
	private Pair<ChannelCreator.ChannelCloseListener, ChannelFuture> createChannelUDP(InetSocketAddress recipient,
			ChannelCreator channelCreator, ChannelHandler handler, IdleStateHandler timeoutHandler,
