...
public final class PeerAddress implements Comparable<PeerAddress>, Serializable {
	...
	// indicates that a relay is used. If its relayed it can be slow (buffered) or
	// holepunching
	...
	private static final int NET4_INTERNAL = 0x4000; // xxxxxxxx x1xxxxxx xxxxxxxx //hopefully we'll never use this for
														// IPv6
	...
	public PeerAddress withRelays(Collection<PeerSocketAddress> relays) {
		...
		return this.relays == relays ? this
				: new PeerAddress(ipInternalSocket, ipInternalNetworkPrefix, ipv4Socket, ipv6Socket, ipv4Flag, ipv6Flag,
						reachable4UDP, reachable4TCP, reachable4UDT, reachable6UDP, reachable6TCP, reachable6UDT,
						peerId, relaySize, slow, holePunching, unreachable, net4Internal, skipIPv4, skipIPv6,
						relayTypes, hashCode, relays);
	}
	...
	/**
	 * Creates a peer address, where the byte array has to be in the rigth format
	 * and in the right size. The new offset can be accessed with offset().
	 *
	 */
	...
	public static Pair<PeerAddress, Integer> decode(final int header, final byte[] array, int offset) {
		...
		final PeerAddress peerAddress = builder.peerId((pair = Number160.decode(array, offset)).element0())
				.hashCode(builder.peerId.hashCode()).build();
	...
	public static PeerAddress decode(final int header, final ByteBuf buf) {
		...
		return builder.peerId(Number160.decode(buf)).hashCode(builder.peerId.hashCode()).build();
	}
	...
	private static PeerAddressBuilder decodeHeader(final PeerAddressBuilder builder, final int header) {
		...
		builder.ipv4Flag((header & IPV4) == IPV4).ipv6Flag((header & IPV6) == IPV6)
				...
				.reachable6UDT((header & REACHABLE6_UDT) == REACHABLE6_UDT).relaySize((header & RELAY_SIZE_MASK) >>> 8)
				.holePunching((header & RELAY_FLAGS_MASK) == HOLE_PUNCHING).slow((header & RELAY_FLAGS_MASK) == SLOW)
				.unreachable((header & UNREACHABLE) == UNREACHABLE)
				.net4Internal((header & NET4_INTERNAL) == NET4_INTERNAL).skipIPv4((header & SKIP_IPV4) == SKIP_IPV4)
				.skipIPv6((header & SKIP_IPV6) == SKIP_IPV6).relayTypes(BitSet.valueOf(tmp));
		return builder;
	}

	/**
	 * @return The size of the serialized peer address, calculated using header
	 *         information only
	 */
	public int size() {
	...
	private int encodeHeader() {
		return (ipv4Flag ? IPV4 : 0) | (ipv6Flag ? IPV6 : 0) | (reachable4UDP ? REACHABLE4_UDP : 0)
				| (reachable4TCP ? REACHABLE4_TCP : 0) | (reachable4UDT ? REACHABLE4_UDT : 0)
				| (reachable6UDP ? REACHABLE6_UDP : 0) | (reachable6TCP ? REACHABLE6_TCP : 0)
				| (reachable6UDT ? REACHABLE6_UDT : 0) | ((relaySize << 8) & RELAY_SIZE_MASK)
				| (holePunching ? HOLE_PUNCHING : 0) | (slow ? SLOW : 0) | (unreachable ? UNREACHABLE : 0)
				| (net4Internal ? NET4_INTERNAL : 0) | (skipIPv4 ? SKIP_IPV4 : 0) | (skipIPv6 ? SKIP_IPV6 : 0)
	...
	public static PeerAddress create(final Number160 peerId, String host, int udpPort, int tcpPort, int udtPort)
	...
	public static PeerAddress create(final Number160 peerId, InetSocketAddress inetSocket) {
		return create(peerId, inetSocket.getAddress(), inetSocket.getPort(), inetSocket.getPort(),
				inetSocket.getPort() + 1);
	}

	public static PeerAddress create(final Number160 peerId, InetAddress inet, int udpPort, int tcpPort, int udtPort) {
		if (inet instanceof Inet4Address) {
			final PeerSocket4Address ps4a = PeerSocket4Address.builder().ipv4(IP.IPv4.fromInet4Address(inet))
					.udpPort(udpPort).tcpPort(tcpPort).udtPort(udtPort).build();
			return builder().peerId(peerId).ipv4Socket(ps4a).build();
		} else {
			final PeerSocket6Address ps6a = PeerSocket6Address.builder().ipv6(IP.IPv6.fromInet6Address(inet))
					.udpPort(udpPort).tcpPort(tcpPort).udtPort(udtPort).build();
			return builder().peerId(peerId).ipv6Socket(ps6a).build();
		}
	}
	...
	public InetSocketAddress createUDPSocket(final PeerAddress other) {
		...
		if ((preferIPv6Addresses && canIPv6) || (!canIPv4 && canIPv6)) {
			if (ipv6Socket == null) {
	...
	/*
	 * private int prefix4(InetAddress inetAddress) { NetworkInterface
	 * networkInterface; try { networkInterface =
	 * NetworkInterface.getByInetAddress(internalPeerSocketAddress.inetAddress());
	 * if(networkInterface == null) { return -1; }
	 * if(networkInterface.getInterfaceAddresses().size() <= 0) { return -1; }
	 * for(InterfaceAddress iface: networkInterface.getInterfaceAddresses()) {
	 * if(iface == null) { continue; } if(iface.getAddress() instanceof
	 * Inet4Address) { return iface.getNetworkPrefixLength(); } } return -1; } catch
	 * (SocketException e) { e.printStackTrace(); return -1; } }
	 */

	/*
	 * public PeerSocketAddress strip() { if(internalPeerSocketAddress == null) {
	 * throw new IllegalArgumentException("internal peer socket must be set"); }
	 * if(internalNetworkPrefix == -1) { return internalPeerSocketAddress; } IP.IPv4
	 * masked = IP.fromInet4Address(internalPeerSocketAddress.inetAddress()); masked
	 * = masked.maskWithNetworkMaskInv(internalNetworkPrefix); final InetAddress
	 * inet = masked.toInetAddress(); return new PeerSocketAddress(inet,
	 * internalPeerSocketAddress.tcpPort(), internalPeerSocketAddress.udpPort()); }
	 */
